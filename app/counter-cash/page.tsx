...
type Counts = Record<Denom, number>;
...
const DENOM_LABELS: Record<Denom, string> = {
  500: 'note', 200: 'note', 100: 'note', 50: 'coin', 20: 'coin', 10: 'coin',
};
...
export default function CounterCashPage() {
  ...
  const [counts, setCounts]     = useState<Counts>({ 500: 0, 200: 0, 100: 0, 50: 0, 20: 0, 10: 0 });
  const [saving, setSaving]     = useState(false);
  const [saved, setSaved]       = useState(false);

  const total = DENOMS.reduce((sum, d) => sum + counts[d] * d, 0);
  ...
  function setCount(denom: Denom, value: number) {
    setCounts(p => ({ ...p, [denom]: Math.max(0, isNaN(value) ? 0 : value) }));
  }

  async function handleSubmit() {
    setSaving(true);
    const body: Record<string, unknown> = { log_date: logDate, staff_id: staff?.id };
    for (const d of DENOMS) body[`count_${d}`] = counts[d];
      ...
      <div className="px-4 pt-4 pb-2 space-y-3">
        ...
        <div className="rounded-2xl overflow-hidden border border-white/10 divide-y divide-white/10">
          {DENOMS.map(denom => {
            ...
            return (
              <div key={denom} className={cn('px-4 py-3 bg-white/5 flex items-center gap-3 transition-colors', active && 'bg-emerald-500/10')}>
                {/* Denomination label */}
                <div className="w-16 shrink-0">
                  <p className="text-white font-bold text-sm">₹{denom}</p>
                  <p className="text-white/30 text-xs">{DENOM_LABELS[denom]}</p>
                </div>
                ...
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <input
                    ...
                    min={0}
                    value={count === 0 ? '' : count}
                    onChange={e => setCount(denom, parseInt(e.target.value || '0', 10))}
                    placeholder="0"
                    className={cn(
          ...
          {total > 0 && (
            <div className="mt-2 flex flex-wrap gap-2">
              {DENOMS.filter(d => counts[d] > 0).map(d => (
                <span key={d} className="text-xs text-emerald-300/70 bg-emerald-500/10 px-2 py-0.5 rounded-full">
                  ₹{d}×{counts[d]}
                </span>
              ))}
            </div>
